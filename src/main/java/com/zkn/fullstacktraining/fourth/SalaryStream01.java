...
public class SalaryStream01 {
    ...
    Function<List<SalaryAssistScope>,SalaryAssistScope> function = salaryFunction->{
        return salaryFunction.stream().collect(Collectors.reducing(
                ...
                (s1,s2)->{s1.setName(s2.getName());s1.setSalaryTotal(s1.getSalaryTotal()+s2.getSalaryTotal());
                    s1.setCount(s1.getCount()+s2.getCount());return s1;}));
    };
    @Test
    public void secondExample(){
        ...
        try {
            ...
            lnReader.lines().parallel()//得到Stream流
                    ...
                    .entrySet().parallelStream() //得到Map中的Map.Entry<String,SalaryAssistScope>
                    //.stream()   //得到Entry流
                    //.parallel() //并行
                    .sorted((first,second) -> Long.compare(second.getValue().getSalaryTotal(),first.getValue().getSalaryTotal()))//排序
                    .limit(10)//取前10个
                    .forEachOrdered(salary -> System.out.println("姓名："+salary.getKey()+" "+salary.getValue().getSalaryTotal()/10000+" 万" + salary.getValue().getCount()+" 人"));//输出
            System.out.println("并行耗时："+(System.currentTimeMillis()-startTime));
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        } catch (FileNotFoundException e) {
