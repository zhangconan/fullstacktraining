...
public class StreamCollectExercises01 {
    ...
    List<Salary> salaries = new ArrayList<Salary>() {
        {
            ...
            add(new Salary("zha12ngsan",71,54));
    ...
    public void testCollect03(){
        //将流中的数据合并到第一个list上
        salaries.stream().reduce((s1,s2)->{s1.setBaseSalary(s1.getBaseSalary()+s2.getBaseSalary());s1.setBonus(s1.getBonus()+s2.getBonus());return s1;})
                .ifPresent(s->System.out.println(s.getName()+"  "+s.getBaseSalary()+"  "+s.getBonus()));
